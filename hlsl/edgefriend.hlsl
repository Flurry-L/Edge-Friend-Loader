...
void CSEdgefriend(uint3 dispatchThreadID : SV_DispatchThreadID)
{
    ...
    */
    ...
    quads[0].x = (quad.x > F) ? (3 * F + quad.x) : (4 * quad.x);
    quads[0].y = edgePointOn0;
    quads[0].z = facePoint;
    quads[0].w = edgePointOff1;

    quads[1].x = (quad.y > F) ? (3 * F + quad.y) : (4 * quad.y);
    quads[1].y = edgePointOff0;
    quads[1].z = facePoint;
    quads[1].w = edgePointOn0;

    quads[2].x = (quad.z > F) ? (3 * F + quad.z) : (4 * quad.z);
    quads[2].y = edgePointOn1;
    quads[2].z = facePoint;
    quads[2].w = edgePointOff0;

    quads[3].x = (quad.w > F) ? (3 * F + quad.w) : (4 * quad.w);
...
void ComputeVertexPoint(int vertex, out int offset, out float3 vertexPoint, out int newValenceStartInfo)
{
    offset = (vertex > F) ? (3 * F + vertex) : (4 * vertex);
    
    int corner = valenceStartInfoBufferIn[vertex];
    if (corner < 0 || corner >= F * 4)
    { // vertex not in use
    ...
    do
    {
        ...
        float sharpness = asfloat(friendAndSharpness[1]) * sharpnessFactor;
